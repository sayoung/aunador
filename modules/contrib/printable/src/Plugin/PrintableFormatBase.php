...
abstract class PrintableFormatBase extends PluginBase implements PrintableFormatInterface, ContainerFactoryPluginInterface {
   ...
   *   The render service.
   */
  public function __construct(array $configuration, $plugin_id, array $plugin_definition, ConfigFactoryInterface $config_factory, PrintableCssIncludeInterface $printable_css_include, LinkExtractorInterface $link_extractor, RendererInterface $renderer) {
  ...
  public function setContent(array $content) {
    ...
    if ($this->configFactory->get('printable.settings')
      ->get('list_attribute')) {
      $this->footerContent = $this->linkExtractor->listAttribute((string) $this->renderer->render($this->content));
    }
  }
   ...
   *
   * @return string
   *   A string representing the output of this printable format (HTML or a file location)
   */
  public function getOutput() {
    $content = $this->buildContent();
    return $this->extractLinks($this->renderer->render($content));
  }
