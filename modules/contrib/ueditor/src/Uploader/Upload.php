...
abstract class Upload {
    ...
    protected function savefileToDrupal($imagedata, $savePath, $managed = FALSE, $replace = FileSystemInterface::EXISTS_RENAME) {
        if($managed) {
          $local = \Drupal::service('file_system')->saveData($imagedata, 'public://'.$savePath, $replace);
        }else {
          try {
              $local = \Drupal::service('file_system')->saveData($imagedata, 'public://'.$savePath, $replace);
          }
    ...
    protected function getFullName() {
        ...
        if(strpos($format, '{transliteration_filename}')){
          if(\Drupal::moduleHandler()->moduleExists('transliteration') && function_exists('transliteration_clean_filename')) {
