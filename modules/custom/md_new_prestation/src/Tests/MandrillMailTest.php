...
/**
 * @file
 * Contains \Drupal\md_new_prestation\Tests\MandrillMailTestTest.
 */

namespace Drupal\md_new_prestation\Tests;
...
class MandrillMailTest extends KernelTestBase {
  ...
  protected function setUp() {
    ...
    \Drupal::service('module_installer')->install(array('md_new_prestation'));
  }
  ...
  function testMandrillEmail() {
    ...
    $this->assertEqual($sent_message['id'], 'md_new_prestation_node_insert', 'Correct mail id.');
  ...
  function doMandrillConfig() {
    ...
    $this->assertTrue(in_array('md_new_prestation', array_keys($config)), 'The Mandrill mailer is present in the configuration.');
  }
  ...
  function doMandrillNoConfig() {
    \Drupal::service('module_installer')->uninstall(array('md_new_prestation'));
    $config = \Drupal::configFactory()->getEditable('system.mail')->get('interface');
    $this->assertTrue(!in_array('md_new_prestation', array_keys($config)), 'The Mandrill mailer is not present in the configuration.');
  }
}
