...
/**
 * @file
 * Contains \Drupal\notemail\Tests\MandrillMailTestTest.
 */
...
class MandrillMailTest extends KernelTestBase {
  ...
  protected function setUp() {
    ...
    \Drupal::service('module_installer')->install(array('notemail'));
  }
  ...
  function testMandrillEmail() {
    ...
    $this->assertEqual($sent_message['id'], 'notemail_node_insert', 'Correct mail id.');
  ...
  function doMandrillConfig() {
    ...
    $this->assertTrue(in_array('notemail', array_keys($config)), 'The Mandrill mailer is present in the configuration.');
  }
  ...
  function doMandrillNoConfig() {
    \Drupal::service('module_installer')->uninstall(array('notemail'));
    $config = \Drupal::configFactory()->getEditable('system.mail')->get('interface');
    $this->assertTrue(!in_array('notemail', array_keys($config)), 'The Mandrill mailer is not present in the configuration.');
  }
}
