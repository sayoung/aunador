...
/**
 * @file
 * Contains \Drupal\dardev_pre_instruction\Tests\MandrillMailTestTest.
 */

namespace Drupal\dardev_pre_instruction\Tests;
...
class MandrillMailTest extends KernelTestBase {
  ...
  protected function setUp() {
    ...
    \Drupal::service('module_installer')->install(array('dardev_pre_instruction'));
  }
  ...
  function testMandrillEmail() {
    ...
    $this->assertEqual($sent_message['id'], 'dardev_pre_instruction_node_insert', 'Correct mail id.');
  ...
  function doMandrillConfig() {
    ...
    $this->assertTrue(in_array('dardev_pre_instruction', array_keys($config)), 'The Mandrill mailer is present in the configuration.');
  }
  ...
  function doMandrillNoConfig() {
    \Drupal::service('module_installer')->uninstall(array('dardev_pre_instruction'));
    $config = \Drupal::configFactory()->getEditable('system.mail')->get('interface');
    $this->assertTrue(!in_array('dardev_pre_instruction', array_keys($config)), 'The Mandrill mailer is not present in the configuration.');
  }
}
