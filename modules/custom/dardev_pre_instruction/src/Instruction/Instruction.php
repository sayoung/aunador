...
namespace Drupal\dardev_pre_instruction\Instruction;
...
class Instruction{
	  const SETTINGS = 'dardev_pre_instruction.settings';

  public static function sendMail($to, $code, $title){
	  $config = \Drupal::config('dardev_pre_instruction.settings');
	  $mailManager = \Drupal::service('plugin.manager.mail');

  $module = 'dardev_pre_instruction';
  $key = 'node_insert';
  $to = $to; 
  ...
  if ( ! $result['result']) {
    ...
    \Drupal::logger('dardev_pre_instruction')->error($message);
    return;
  }

  $message = t('An email notification has been sent to @email for creating node @id.');
//  \Drupal::messenger()->addMessage($message);
  \Drupal::logger('dardev_pre_instruction')->notice($message);
