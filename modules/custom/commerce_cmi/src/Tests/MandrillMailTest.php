...
/**
 * @file
 * Contains \Drupal\commerce_cmi\Tests\MandrillMailTestTest.
 */

namespace Drupal\commerce_cmi\Tests;
...
class MandrillMailTest extends KernelTestBase {
  ...
  protected function setUp() {
    ...
    \Drupal::service('module_installer')->install(array('commerce_cmi'));
  }
  ...
  function testMandrillEmail() {
    ...
    $this->assertEqual($sent_message['id'], 'commerce_cmi_node_insert', 'Correct mail id.');
  ...
  function doMandrillConfig() {
    ...
    $this->assertTrue(in_array('commerce_cmi', array_keys($config)), 'The Mandrill mailer is present in the configuration.');
  }
  ...
  function doMandrillNoConfig() {
    \Drupal::service('module_installer')->uninstall(array('commerce_cmi'));
    $config = \Drupal::configFactory()->getEditable('system.mail')->get('interface');
    $this->assertTrue(!in_array('commerce_cmi', array_keys($config)), 'The Mandrill mailer is not present in the configuration.');
  }
}
