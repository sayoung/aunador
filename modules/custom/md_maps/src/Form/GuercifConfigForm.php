 ...
 */
class GuercifConfigForm extends ConfigFormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'maps_guercif_settings_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
	...
	$config = $this->config("md_maps_prov_g.setting");
    for($i=1; $i<=11; $i++){
    $form['md_maps_prov_g']['title_'. $i] = array(
	...
	$form['md_maps_prov_g']['info_'. $i] = array(
      '#type' => 'text_format',
      '#format' => 'full_html',
	  '#suffix' => '</div></div>',
      '#attributes' => array('class' => array('text-input', 'mauticform-input'), 'placeholder' => $this->t('Remplire le champs')),
...
for($i=1; $i<=11; $i++){
	   $this->configFactory->getEditable("md_maps_prov_g.setting")
